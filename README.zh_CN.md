...
| 日期 | 版本 | 说明 |
| :--- | :--- | :--- |
| 2026-09-10 | 1.2.2 | 修复：手动选择的“界面”不再被动作类型（文件夹/收藏/插件/补丁/录制）默认值或卡片重建覆盖 |
| 2026-09-10 | 1.2.1 | 修复：插件/补丁类动作的子菜单路径回放（索引失效时按已记录标题路径回退）并细化错误提示；切换“快捷操作菜单 / 添加到按钮”不再把已输入名称恢复为默认名；控制中心“按钮形状/按钮背景”与“按钮大小/标签大小”改为同行两格 |
| 2026-09-10 | 1.2.0 | 移除动作类型图标，改为状态前缀 `≡`（已加入快捷操作菜单）/ `⊚`（已保存为按钮）；两卡新增“添加到按钮”开关；清理僵尸按钮；控制中心菜单重排（同行两格、移除“添加按钮”）；“编辑按钮”列出并移除已保存按钮；预设长按=应用该预设；修复崩溃（helper 前向声明、配置 `reloadConfig` / `replaceConfig`） |
| 2026-09-06 | 1.1.0 | 插件形态（本仓库首个版本）：补丁 → 插件迁移、拆分为 `qc_*` 叶子模块、标准菜单入口（`addToMainMenu`）、字体补丁套娃修复、移除 `Dispatcher.execute` 全局重写、luacheck 与单元测试 |
| 2026-08-24 | 补丁 | `2-quickcenter.lua` 深度重构与修复：体积 −17%、Dispatcher/Nerd Font 缓存 O(n²) → O(n)、3 处潜在崩溃修复、动作执行 `pcall` 兜底（历史版本，见补丁时代提交） |
