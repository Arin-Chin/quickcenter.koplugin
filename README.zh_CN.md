...
| **编辑操作** | 新增 / 重命名 / 删除；已有自定义操作归入 **已有操作**；勾选 **添加到快捷操作菜单** 提升为快捷方式 |
| **快捷操作菜单** | 管理已添加的快捷方式（勾选/取消勾选以移除）；手势可唤出无标题栏的运行列表 |
...
| **添加 / 移除按钮** | 以复选框从可用动作中选择面板按钮（QuickCenter 样式框，含返回导航） |
...
| :--- | :--- |
| **保存配置** | 将当前配置保存为**命名预设** |
| **编辑配置** | 浏览已保存预设：**长按以当前配置覆盖**，或进入子菜单进行更新 / 重命名 / 删除 |
| **重设配置** | 恢复出厂默认值 |
